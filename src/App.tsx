...
export default function App() {
  ...
  return (
    <div className="w-full h-full min-h-screen">
      ...
      {currentView === "result" && (
        <AiResult navigate={setCurrentView} uploadedImage={uploadedImage} />
      )}
      {currentView === "result2" && (
        <AiResult2 navigate={setCurrentView} uploadedImage={uploadedImage} />
      )}
      {currentView === "result3" && (
        <AiResult3 navigate={setCurrentView} uploadedImage={uploadedImage} />
      )}
      {currentView === "result4" && (
        <AiResult4 navigate={setCurrentView} uploadedImage={uploadedImage} />
      )}
      {currentView === "result5" && (
        <AiResult5 navigate={setCurrentView} uploadedImage={uploadedImage} />
      )}
      {currentView === "result6" && (
        <AiResult6 navigate={setCurrentView} uploadedImage={uploadedImage} />
      )}
      {currentView === "result7" && (
        <AiResult7 navigate={setCurrentView} uploadedImage={uploadedImage} />
      )}
      {currentView === "atlas" && <Atlas navigate={setCurrentView} />}
      {currentView === "library" && <Library navigate={setCurrentView} />}
      {currentView.startsWith("reader:") && (
        <Reader
          navigate={setCurrentView}
          bookId={currentView.split(":", 2)[1]}
        />
      )}
      ...
      {currentView.startsWith("games:level:") && (
        <GameLevel
          navigate={setCurrentView}
          level={parseInt(currentView.split(":", 3)[2])}
        />
      )}
      {currentView === "games:success" && (
        <GameSuccess navigate={setCurrentView} />
      )}
    </div>
  );
}
